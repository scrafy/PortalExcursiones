...
namespace PortalExcursiones.Infraestructura.ImplementacionInterfaces
{
    public class ClienteOperacionesComunes : IOperacionesComunes<cliente>
    {
            ...
            try
            {
                if(modelo.IsValid)
                {
                    ...
                    else
                    {
                        ...
                        resp.Mensaje = Enum.GetName(typeof(Codigos), (int)Codigos.OK);
                        resp.Mensaje_error = resp.Mensaje_error = String.Format("El usuario con ID {0} no se ha podido encontrar", Entidad.usuario.Id);
                        return resp.ObjectoRespuesta();
                    }
                }
                else
        ...
        public HttpResponseMessage Todos()
        {
            try
            {
                var clientes = contexto.cliente.Include("usuario").Select(x => new
                {
                    id = x.usuario_id,
                    ...
                    telefono1 = x.usuario.PhoneNumber,
        ...
        public HttpResponseMessage BusquedaPorId(string id)
        {
            try
            {
                var cliente = contexto.cliente.Include("usuario").Where(x => x.usuario_id == id).Select(x => new
                {
                    id = x.usuario_id,
                    numero_identificacion = x.numidentificacion,
                    inf_adicional = x.infadicional,
                ...
                else
                {
                    ...
                    resp.Mensaje_error = resp.Mensaje_error = String.Format("El usuario con ID {0} no se ha podido encontrar", id);
                    return resp.ObjectoRespuesta();
                }
            }
            catch (Exception ex)
